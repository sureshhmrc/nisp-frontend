...
class ApplicationConfig @Inject()(configuration: Configuration) extends ServicesConfig {
  ...
  lazy val showGovUkDonePage: Boolean = configuration.getBoolean("govuk-done-page.enabled").getOrElse(true)
  lazy val govUkFinishedPageUrl: String = loadConfig("govuk-done-page.url")
  lazy val identityVerification: Boolean = configuration.getBoolean("microservice.services.features.identityVerification").getOrElse(false)
  ...
  lazy val ivUpliftUrl: String = configuration.getString(s"identity-verification-uplift.host").getOrElse("")
  lazy val ggSignInUrl: String = configuration.getString(s"government-gateway-sign-in.host").getOrElse("")

  lazy val showUrBanner:Boolean = configuration.getBoolean("urBannerToggle").getOrElse(false)
  val GaEventAction: String = "home page UR"

  private lazy val pertaxFrontendService: String = baseUrl("pertax-frontend")
  ...
  lazy val isWelshEnabled = configuration.getBoolean("microservice.services.features.welsh-translation").getOrElse(false)
  lazy val feedbackFrontendUrl: String = loadConfig("feedback-frontend.url")
}
