...
import uk.gov.hmrc.nisp.helpers.{MockStatePensionConnection, MockStatePensionService, TestAccountBuilder}
...
class StatePensionServiceSpec extends UnitSpec with ScalaFutures {
  ...
  "StatePensionConnection" should {
    ...
    "transform the MCI 403 into a Left(StatePensionExclusion(MCI))" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.excludedAllButDead)) { exclusion =>
    ...
    "return the connector response for a regular user" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.regularNino)) { statePension =>
    ...
    "return the connector response for a RRE user" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.excludedMwrre)) { statePension =>
        statePension shouldBe Right(StatePension(
          new LocalDate(2015, 4, 5),
          StatePensionAmounts(
            protectedPayment = false,
    ...
    "return the connector response for a Abroad user" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.excludedAbroad)) { statePension =>
        statePension shouldBe Right(StatePension(
          new LocalDate(2015, 4, 5),
          StatePensionAmounts(
            protectedPayment = false,
    ...
    "return the connector response with PostStatePensionAge exclusion for all the exclusions except MCI and Dead" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.excludedAllButDeadMCI)) { statePension =>
    ...
    "return the connector response for a user with a true flag for State Pension Age Under Consideration" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.spaUnderConsiderationNino)) { statePension =>
        statePension shouldBe Right(StatePension(
          new LocalDate(2015, 4, 5),
          StatePensionAmounts(
            protectedPayment = false,
    ...
    "return the connector response for a user with no flag for State Pension Age Under Consideration" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.spaUnderConsiderationNoFlagNino)) { statePension =>
        statePension shouldBe Right(StatePension(
          new LocalDate(2015, 4, 5),
          StatePensionAmounts(
            protectedPayment = false,
    ...
    "return the connector response for a user with exclusion with a true flag for State Pension Age Under Consideration" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.spaUnderConsiderationExclusionIoMNino)) { statePension =>
        statePension shouldBe Left(StatePensionExclusionFiltered(
          Exclusion.IsleOfMan,
          pensionAge = Some(65),
          pensionDate = Some(new LocalDate(2017, 7, 18)),
    ...
    "return the connector response for a user with exclusion with no flag for State Pension Age Under Consideration" in {
      whenReady(MockStatePensionConnection.getSummary(TestAccountBuilder.spaUnderConsiderationExclusionNoFlagNino)) { statePension =>
        statePension shouldBe Left(StatePensionExclusionFiltered(
          Exclusion.IsleOfMan,
          pensionAge = Some(65),
          pensionDate = Some(new LocalDate(2017, 7, 18)),
