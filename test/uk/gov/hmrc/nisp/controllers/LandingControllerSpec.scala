...
import uk.gov.hmrc.nisp.helpers.{MockAuthAction, MockCachedStaticHtmlPartialRetriever, MockIdentityVerificationConnector, TestAccountBuilder}
...
class LandingControllerSpec extends PlaySpec with MockitoSugar with OneAppPerSuite {
  ...
  def testLandingController(nino: Nino): LandingController = new LandingController {
  ...
  "GET /" should {
    "return 200" in {
      val result = testLandingController(TestAccountBuilder.regularNino).show(fakeRequest)
    ...
    "return IVLanding page" in {
      Mockito.when(testLandingController(TestAccountBuilder.regularNino).applicationConfig.isWelshEnabled).thenReturn(false)
      val result = testLandingController(TestAccountBuilder.regularNino).show(fakeRequest)
    ...
    "return non-IV landing page when switched on" in {
      ...
      Mockito.when(testLandingController(TestAccountBuilder.regularNino).applicationConfig.isWelshEnabled).thenReturn(false)
      Mockito.when(testLandingController(TestAccountBuilder.regularNino).applicationConfig.identityVerification).thenReturn(true)
      val result = testLandingController(TestAccountBuilder.regularNino).show(fakeRequest)
  ...
  "GET /signin/verify" must {
    "redirect to verify" in {
      val result = testLandingController(TestAccountBuilder.regularNino).verifySignIn(fakeRequest)
    ...
    "redirect to account page when signed in" in {
      val result = testLandingController(TestAccountBuilder.regularNino).verifySignIn(FakeRequest().withSession(
  ...
  "GET /not-authorised" must {
    "show not authorised page" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(None)(fakeRequest)
    ...
    "show generic not_authorised template for FailedMatching journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("failed-matching-journey-id"))(fakeRequest)
      contentAsString(result) must include("We cannot confirm your identity")
    }

    "show generic not_authorised template for InsufficientEvidence journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("insufficient-evidence-journey-id"))(fakeRequest)
      contentAsString(result) must include("We cannot confirm your identity")
    }

    "show generic not_authorised template for Incomplete journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("incomplete-journey-id"))(fakeRequest)
      contentAsString(result) must include("We cannot confirm your identity")
    }

    "show generic not_authorised template for PreconditionFailed journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("precondition-failed-journey-id"))(fakeRequest)
      contentAsString(result) must include("We cannot confirm your identity")
    }

    "show generic not_authorised template for UserAborted journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("user-aborted-journey-id"))(fakeRequest)
      contentAsString(result) must include("We cannot confirm your identity")
    }

    "show technical_issue template for TechnicalIssue journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("technical-issue-journey-id"))(fakeRequest)
    ...
    "show locked_out template for LockedOut journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("locked-out-journey-id"))(fakeRequest)
    ...
    "show timeout template for Timeout journey" in {
      val result = testLandingController(TestAccountBuilder.regularNino).showNotAuthorised(Some("timeout-journey-id"))(fakeRequest)
  ...
  "GET /cymraeg" must {
    ...
    "return 200" in {
      val result = testLandingController(TestAccountBuilder.regularNino).show(fakeRequestWelsh)
      status(result) mustBe Status.OK
    }
